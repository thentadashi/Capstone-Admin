	...
	function doEdit(){
		...
		if ($_GET['actions']=='confirm') {
				...
				$remarks ='your order is on the way by our trusted Couriers.';
							...
							# code...
				$status	= 'not';	
				$remarks ='sorry its seems like you are still not verify';
	        ...
	        $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$cur->PHONE."','IT MAN','From Urdaneta City : Your order has been '".$status. "'. The amount is '".$cur->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
			...
			foreach ($cur as $result) {
	  		 ...
	  		 $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$result->OWNERPHONE."','IT MAN','From Urdaneta City : Your  product has been ordered'. The amount is '".$result->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
